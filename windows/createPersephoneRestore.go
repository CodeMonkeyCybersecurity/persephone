...
const RESTORE_TARGET = `C:\persephoneRestore`

// Snapshot represents a persephone snapshot.
...
// listSnapshots calls persephone to list snapshots in JSON format and returns them.
func listSnapshots(repoFile, passFile string) ([]Snapshot, error) {
	...
	if err := json.Unmarshal(output, &snapshots); err != nil {
		return nil, fmt.Errorf("error parsing JSON output from persephone: %v", err)
	}
	return snapshots, nil
}

// displaySnapshots prints a numbered list of snapshots and returns a slice of snapshot short IDs.
...
func restoreSnapshot(repoFile, passFile, snapshotID, target string) {
	reader := bufio.NewReader(os.Stdin)
	...
	cmd := exec.Command("restic",
		"--repository-file", repoFile,
		"--password-file", passFile,
		"restore", snapshotID, "--target", target,
	)
...
func main() {
	...
	fmt.Println("Checking Persephone backup and snapshots...\n")
	...
	restoreSnapshot(repoFile, passFile, selectedSnapshot, RESTORE_TARGET)
	fmt.Println("Persephone restore process complete.")
}
